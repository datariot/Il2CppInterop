...
namespace Il2CppInterop.Runtime.Injection
{
    internal static unsafe class InjectorHelpers
    {
        ...
        private static IntPtr GetIl2CppModuleBaseAddress()
        {
            ...
            {
                // arm64-macOS: dladdr on the il2cpp_init export returns 0 unreliably here. Find the loaded
                // GameAssembly.dylib's base by iterating dyld's image list directly.
                ...
                for (uint i = 0; i < count; i++)
                {
                    ...
                    if (name != null && name.EndsWith("GameAssembly.dylib", StringComparison.Ordinal))
                        return _dyld_get_image_header(i);
                }
                throw new InvalidOperationException("GameAssembly.dylib not found in loaded dyld images");
            }
        ...
        [DllImport("libSystem.dylib", EntryPoint = "_dyld_image_count", CallingConvention = CallingConvention.Cdecl)]
        private static extern uint _dyld_image_count();

        [DllImport("libSystem.dylib", EntryPoint = "_dyld_get_image_header", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr _dyld_get_image_header(uint image_index);

        [DllImport("libSystem.dylib", EntryPoint = "_dyld_get_image_name", CallingConvention = CallingConvention.Cdecl)]
        private static extern IntPtr _dyld_get_image_name(uint image_index);
    }
}
